...
def cluster(blastdb, taxdb):
    ...
    for iter in c.execute("SELECT Gene_name from blast WHERE decision = 'To cluster analysis/Chosen' GROUP BY Gene_name"):
        genes.append(iter[0])
    ...
    for gene in genes:
        ...
        multfinalseqs = []
        unresolved = []
        for iter in c.execute("SELECT tc_id, GI from blast where decision = 'To cluster analysis/Chosen' AND Gene_name = '"+gene+"'order by tc_id"):
